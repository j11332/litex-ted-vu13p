...
from bench.platform import Platform
...
class SimSoC(SoCCore):
    ...
    def __init__(self,
        ...
        **kwargs):
        ...
        self.add_config("SIM_DMA_TEST", 1)
    def _add_kyokko(self, platform):
        ...
        self.submodules.k2mm_qsfp0 = k2mm_qsfp0 = K2MM(dw=256, id=0x0400)
        self.comb += [
            kyokko.source_user_rx.connect(k2mm_qsfp0.sink_packet_rx, omit={"error", "src_port", "dst_port", "ip_address", "length"}),
            k2mm_qsfp0.source_packet_tx.connect(kyokko.sink_user_tx, omit={"error", "src_port", "dst_port", "ip_address", "length"}),
        ]
        ...
        self.submodules.k2mm_qsfp1 = k2mm_qsfp1 = K2MM(dw=256, id=0x0401)
        self.comb += [
            ky1.source_user_rx.connect(k2mm_qsfp1.sink_packet_rx, omit={"error", "src_port", "dst_port", "ip_address", "length"}),
            k2mm_qsfp1.source_packet_tx.connect(ky1.sink_user_tx, omit={"error", "src_port", "dst_port", "ip_address", "length"}),
        ]
...
def generate_gtkw_savefile(builder, vns, trace_fst):
    ...
    with gtkw.GTKWSave(vns, savefile=savefile, dumpfile=dumpfile) as save:
        save.clocks()
        save.fsm_states(soc)
        if hasattr(soc, "trace_targets"):
            for endpoint in soc.trace_targets:
                save.add(endpoint)
...
def main():
    ...
    builder = LocalBuilder(soc, **builder_kwargs)
    builder.build(
