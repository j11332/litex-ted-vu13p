...
class XilinxILATracer:
    ...
    def __init__(self, platform = None):
        if platform is None and XilinxILATracer.platform is None:
        ...
        platform.toolchain.pre_synthesis_commands += [
            ...
            "set_property FILE_TYPE TCL [get_files *.tcl -filter \"IS_GENERATED == false\"]",
        ]
    ...
    def add_probe(self, sig, clock, trigger=False):
        if XilinxILATracer.cores.get(clock.duid) is None:
            clock.attr.add(('ILA_CLOCK_DUID', str(clock.duid)))
            clock.attr.add(('KEEP', 'true'))
            XilinxILATracer.cores[clock.duid] = []
        ...
        XilinxILATracer.cores[clock.duid] += [(sig, "DATA_AND_TRIGGER" if trigger else "DATA")]
    
    def generate_ila(self, path):
        commands = [ "# Generate ILA cores" ]
        for k, v in XilinxILATracer.cores.items():
            commands += [
            ...
            for i, tsig in enumerate(v):
                ...
                commands += [
                    "set_property -dict {"
                        "PROBE_TYPE " f"{tsig[1]} "
                        "PORT_WIDTH " f"{len(sig)}"
                        "} "
                        f"[get_debug_ports ila_{k}/probe{i}]",
